...
#include "gdn_blockdv_study_kernel.cuh"
...
void launch_gdn_prefill_bf16_gva_blockdv(
  ...
  if (tile_tokens != 64 || block_dv != 64) {
    throw std::runtime_error("block-DV study currently instantiates --tile 64 --block-dv 64");
  }

  if (variant_id == 0) {
    launch_gdn_prefill_bf16_gva_blockdv<64, 64, 2, 3, 2>(
  ...
  if (variant_id == 1) {
    launch_gdn_prefill_bf16_gva_blockdv<64, 64, 2, 2, 2>(
        stream, output, output_state, q, k, v, alpha, beta, cu_seqlens, workspace, num_seqs,
        num_q_heads, num_k_heads, num_v_heads, num_o_heads, head_size, total_seqlen, scale,
        sm_count);
    return;
  ...
  if (variant_id == 2) {
    launch_gdn_prefill_bf16_gva_blockdv<64, 64, 3, 3, 2>(
        stream, output, output_state, q, k, v, alpha, beta, cu_seqlens, workspace, num_seqs,
        num_q_heads, num_k_heads, num_v_heads, num_o_heads, head_size, total_seqlen, scale,
        sm_count);
    return;
  ...
  if (variant_id == 3) {
    launch_gdn_prefill_bf16_gva_blockdv<64, 64, 2, 3, 3>(
        stream, output, output_state, q, k, v, alpha, beta, cu_seqlens, workspace, num_seqs,
        num_q_heads, num_k_heads, num_v_heads, num_o_heads, head_size, total_seqlen, scale,
        sm_count);
    return;
