...
void launch_gdn_prefill_bf16_gva_state_only_checkpoint(
    ...
    float const* alpha, float const* beta, int64_t const* cu_seqlens, uint8_t* workspace_buffer,
  ...
  using Options = std::tuple<
      ...
      Option<Tag::kEnableCheckpointing, cute::true_type>>;

  using TileShape = Shape<_64, _64, _128>;
  using Scheduler = cutlass::gemm::KernelTmaWarpSpecializedCooperative;
  using Operation = cutlass::device::Universal<typename flat::kernel::FlatBuilderDeltaRuleStateOnly<
  ...
  (void)checkpoint_cu_starts;
  (void)checkpoint_every_n_tokens;
