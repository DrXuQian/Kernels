...
struct WorkDesc {
  ...
  int32_t private_v_block_idx;
  int32_t private_v_dim_offset;
  ...
  CUTE_DEVICE int32_t v_block_idx() const { return private_v_block_idx; }
  CUTE_DEVICE int32_t v_dim_offset() const { return private_v_dim_offset; }
...
template <typename GroupingTag = GQATag, int BlockDV = 64>
struct IndividualTileScheduler {
  ...
  static Params to_underlying_arguments(ProblemSize const& problem_size,
    ...
    dim3 grid(0, 1, 1);
    int32_t const block_dv = BlockDV;
    int32_t const v_dim_blocks =
        (problem_size.head_size + block_dv - 1) / block_dv;
    if constexpr (std::is_same_v<GroupingTag, GQATag>) {
      grid.x = problem_size.num_seqs * problem_size.num_q_heads * v_dim_blocks;
    } else if constexpr (std::is_same_v<GroupingTag, GVATag>) {
      grid.x = problem_size.num_seqs * problem_size.num_v_heads * v_dim_blocks;
    } else {
      static_assert(dependent_false<GroupingTag>, "unknown grouping relation");
    }
    DPRINTF(
        "to_underlying_arguments: grid:{.x:%d, .y:%d, .z:%d}, num_seqs:%d, num_q_heads:%d, "
        "num_v_heads:%d, v_dim_blocks:%d, block_dv:%d\n",
        grid.x, grid.y, grid.z, problem_size.num_seqs, problem_size.num_q_heads,
        problem_size.num_v_heads, v_dim_blocks, block_dv);
    return {
        .grid = grid,
        .num_seqs = problem_size.num_seqs,
        .num_q_heads = problem_size.num_q_heads,
        .num_v_heads = problem_size.num_v_heads,
        .v_dim_blocks = v_dim_blocks,
        .block_dv = block_dv,
    };
  }
  ...
  CUTE_DEVICE WorkDesc<GroupingTag> get_next_work(Params params, ProblemSize const& problem_size) {
    ...
    int32_t v_block_idx;
    if constexpr (std::is_same_v<GroupingTag, GQATag>) {
      int32_t const heads_times_blocks = params.num_q_heads * params.v_dim_blocks;
      int32_t const rem = blockIdx.x % heads_times_blocks;
      seq_idx = blockIdx.x / heads_times_blocks;
      q_head_idx = rem / params.v_dim_blocks;
      v_block_idx = rem % params.v_dim_blocks;
      v_head_idx = q_head_idx / (params.num_q_heads / params.num_v_heads);
    } else if constexpr (std::is_same_v<GroupingTag, GVATag>) {
      int32_t const heads_times_blocks = params.num_v_heads * params.v_dim_blocks;
      int32_t const rem = blockIdx.x % heads_times_blocks;
      seq_idx = blockIdx.x / heads_times_blocks;
      v_head_idx = rem / params.v_dim_blocks;
      v_block_idx = rem % params.v_dim_blocks;
    ...
    if (scheduled) {
      ...
      scheduled = true;
      DPRINTF0_W(
          "get_next_work: this_work={seq_idx:%d q_head_idx:%d v_head_idx:%d v_block:%d "
          "v_offset:%d tok_offset:%lld seq_len:%lld}\n",
          seq_idx, q_head_idx, v_head_idx, v_block_idx, v_block_idx * params.block_dv, s, seq_len);
    }
    ...
    return {
        ...
        .private_v_block_idx = v_block_idx,
        .private_v_dim_offset = v_block_idx * params.block_dv,
        .tok_offset = s,
        .seq_len = seq_len,
    };
  }
