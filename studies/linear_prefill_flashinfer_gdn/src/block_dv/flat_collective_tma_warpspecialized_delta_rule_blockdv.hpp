...
#include "block_dv/flat_collective_load_blockdv.hpp"
#include "block_dv/flat_collective_store_blockdv.hpp"
...
template <class Element_, class ElementAccumulatorQK_, class ElementAccumulatorKV_,
          class TileShape_, int BlockDV_,  // TileShape = (seqlen_q, seqlen_kv, d_qk)
...
struct FlatMainloopTmaWarpSpecializedDeltaRule {
  ...
  static constexpr auto HeadSizeQK = get<2>(TileShape{});  // Dq/Dk
  static constexpr auto HeadSizeV = Int<BlockDV_>{};       // V/O slice per CTA
  ...
  using TileShapeKLoad = decltype(make_shape(HeadSizeQK, HeadSizeQK, BlkSeqKV));
  ...
  using CollectiveMmaK_G2S = typename cutlass::gemm::collective::CollectiveBuilder<
      ...
      Element, decltype(select<1, 0, 2>(LayoutK{})), Alignment, ElementAccumulatorKV, TileShapeKLoad,
      ClusterShape, DummyStages, cutlass::gemm::KernelTmaWarpSpecializedCooperative>::CollectiveOp;

  using CollectiveMmaV_G2S = typename cutlass::gemm::collective::CollectiveBuilder<
      cutlass::arch::Sm90, cutlass::arch::OpClassTensorOp, Element,
      decltype(select<1, 0, 2>(LayoutV{})), Alignment,  // direct TMA copy for GMEM -> SMEM
  ...
  using SmemLayoutK_DS = decltype(unstage_smem_layout(typename CollectiveMmaK_G2S::SmemLayoutB{},
                                                      Int<StagesK::value>{}));
  using SmemLayoutV_DS = decltype(unstage_smem_layout(typename CollectiveMmaV_G2S::SmemLayoutA{},
                                                      Int<StagesV::value>{}));
  ...
  struct SharedStorage {
    ...
    // Study-only guard padding. The block-DV O path uses a sliced epilogue tile,
    // and compute-sanitizer shows the generated smem store can touch a small
    // swizzle/alignment tail beyond cute::cosize_v<SmemLayoutO> for multi-block
    // sequences. Keep the prototype safe while preserving the production path.
    alignas(128) cute::array_aligned<uint8_t, 32 * 1024> block_dv_padding;
  };

  using TMA_Q = typename CollectiveMmaQK::Params::TMA_A;
  using TMA_K = typename CollectiveMmaK_G2S::Params::TMA_B;
  using TMA_V = typename CollectiveMmaV_G2S::Params::TMA_A;
  ...
  static bool can_implement(ProblemShape const& problem_size, Arguments const& args) {
    ...
    return true && ((!IsGVAEnabled && is_gqa_like) || (IsGVAEnabled && is_gva_like)) &&
           (problem_size.head_size == HeadSizeQK) &&
           (problem_size.head_size % HeadSizeV == 0) &&
  ...
  static Params to_underlying_arguments(ProblemShape const& problem_size, Arguments const& args,
    ...
    auto params_kv_k = CollectiveMmaK_G2S::to_underlying_arguments(
        make_shape(d, d, s, problem_size.num_k_heads),
        typename CollectiveMmaK_G2S::Arguments{
            args.ptr_V, select<1, 0, 2>(args.dV),  // not used
            args.ptr_K, select<1, 0, 2>(args.dK),  // used as G2S for K
        },
        /*workspace=*/nullptr);

    auto params_kv_v = CollectiveMmaV_G2S::to_underlying_arguments(
        make_shape(d, d, s, problem_size.num_v_heads),
        typename CollectiveMmaV_G2S::Arguments{
  ...
  CUTE_DEVICE void load_qkv(Params const& params, ProblemShape const& problem_size,
    ...
    using LoadTileShapeQK = decltype(make_shape(BlkSeqQ, BlkSeqKV, HeadSizeQK));
    using LoadTileShapeV = decltype(make_shape(BlkSeqQ, BlkSeqKV, HeadSizeV));
    auto q_src_dst = q_collective_load.partition_SD(problem_size, LoadTileShapeQK{}, work_desc);
    auto k_src_dst = k_collective_load.partition_SD(problem_size, LoadTileShapeQK{}, work_desc);
    auto v_src_dst = v_collective_load.partition_SD(problem_size, LoadTileShapeV{}, work_desc);
  ...
  CUTE_DEVICE void store(TMA_O const& tma_store, void* tensormaps, ProblemSize const& problem_size,
    ...
    using StoreTileShapeV = decltype(make_shape(BlkSeqQ, BlkSeqKV, HeadSizeV));
    auto src_dst = collective_store.partition_SD(problem_size, StoreTileShapeV{}, work_desc);
  ...
  CUTE_DEVICE void compute(
    ...
    auto kv_load = [&](auto& tKVrKV) INLINE_LAMBDA {
      ...
      int64_t state_head_stride = int64_t(problem_size.head_size) * HeadSizeQK;
      int64_t state_seq_stride = int64_t(num_state_heads) * state_head_stride;
      // The block-DV fragment/copy layout carries the V-slice coordinates. Adding
      // v_dim_offset here double-offsets the final slice and overruns state.
      int64_t state_offset = int64_t(seq_idx) * state_seq_stride +
                             int64_t(state_head_idx) * state_head_stride;
      auto gKV = make_tensor(make_gmem_ptr(params.ptr_input_state + state_offset),
    ...
    auto kv_store = [&]() INLINE_LAMBDA {  // tKVrKV is carried over whole mainloop
      ...
      int64_t state_head_stride = int64_t(problem_size.head_size) * HeadSizeQK;
      int64_t state_seq_stride = int64_t(num_state_heads) * state_head_stride;
      // See kv_load: the copy layout already accounts for the V-slice.
      int64_t state_offset = int64_t(seq_idx) * state_seq_stride +
                             int64_t(state_head_idx) * state_head_stride;
      auto gKV = make_tensor(make_gmem_ptr(params.ptr_output_state + state_offset),
                             make_layout(make_shape(Int<HeadSizeQK>{}, Int<HeadSizeV>{})));
    ...
    auto kv_checkpoint_store = [&](int checkpoint_idx) INLINE_LAMBDA {
      if constexpr (kEnableCheckpointing) {
        ...
        // Layout: [total_checkpoints, num_sab_heads, HeadSizeQK, full HeadSizeV] LayoutLeft.
        int64_t state_head_stride = int64_t(problem_size.head_size) * HeadSizeQK;
        int64_t state_ckpt_stride = int64_t(num_state_heads) * state_head_stride;
        int64_t state_offset = ckpt_offset * state_ckpt_stride +
                               int64_t(state_head_idx) * state_head_stride;
        auto gKV = make_tensor(make_gmem_ptr(params.ptr_state_checkpoints + state_offset),
                               make_layout(make_shape(Int<HeadSizeQK>{}, Int<HeadSizeV>{})));
