...
#include "block_dv/flat_kernel_builder_delta_rule_blockdv.hpp"

namespace gdn_blockdv_study {

using namespace cute;

template <int TileTokens, int BlockDV, int StagesQCount, int StagesKCount, int StagesVCount>
void launch_gdn_prefill_bf16_gva_blockdv(
  ...
  static_assert(BlockDV == 64 || BlockDV == 32, "block-DV study supports 64 or 32");
  ...
  using Operation = cutlass::device::Universal<typename flat::kernel::FlatBuilderDeltaRule<
      T, float, float, TileShape, BlockDV,
