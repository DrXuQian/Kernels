// Isolated FlashInfer GDN prefill tile-shape study.
// Usage:
//   ./bench_gdn_tile_study [seqlen] [num_q_heads] [num_v_heads] [head_dim] [num_seqs] --tile 64 [--bench W I]
...
int main(int argc, char** argv) {
  ...
  if (tile_tokens != 64) {
    fprintf(stderr, "unsupported --tile=%d; this study only compiles legal tile 64 variants\n", tile_tokens);
    return 1;
  }
  if (head_dim != 128) {
  ...
  printf("bench gdn_prefill tile study: seqlen=%d seqs=%d q_heads=%d k_heads=%d v_heads=%d dim=%d tile=%d variant=%s\n",
  ...
  try {
    timer.run([&]() {
      gdn_tile_study::launch_gdn_prefill_bf16_gva_tile(
          tile_tokens, variant_id, 0, d_o, d_state, d_q, d_k, d_v, d_alpha, d_beta, d_cu_seqlens,
