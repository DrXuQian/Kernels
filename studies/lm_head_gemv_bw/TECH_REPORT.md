...
| `ptx_tma_ws` | Hopper TMA + warp specialization (sm_90a). 1 producer warp issues `cp.async.bulk.tensor.2d` for an entire R-row x TileK weight tile, mbarriers sync with N consumer warps that FMA from the smem ring. In-flight load data fully in smem; producer doesn't issue per-lane loads. 32 registers, case self-checks correctness. |
