...
```bash
./bench_all.sh --case w4a16_decode_fpA_intB
```

Equivalent aliases:

```bash
./bench_all.sh --kernel w4a16_decode_fpA_intB
./bench_all.sh --only w4a16_decode_fpA_intB
```
...
```bash
BENCH_RUN_ID=my_single_case ./bench_all.sh --case w4a16_decode_fpA_intB
```

Set an explicit output directory:

```bash
OUT_DIR=$PWD/.bench_logs/my_single_case ./bench_all.sh --case w4a16_decode_fpA_intB
```
...
```bash
RUN_ID=w4a16_decode_fpA_intB_$(date +%Y%m%d_%H%M%S)
...
```bash
RUN_DIR=<PERF_MODEL_DIR> ./bench_all.sh --case w4a16_decode_fpA_intB
```

Equivalent:

```bash
PERF_MODEL_DIR=<PERF_MODEL_DIR> ./bench_all.sh --case w4a16_decode_fpA_intB
```
...
```bash
PERFRAWLOG_POSTPROCESS=0 ./bench_all.sh --case w4a16_decode_fpA_intB
PERF_STATISTICS_MP=16 ./bench_all.sh --case w4a16_decode_fpA_intB
PERF_STATISTICS_GHZ=1.5 ./bench_all.sh --case w4a16_decode_fpA_intB
PERF_STATISTICS_SUMMARY=0 ./bench_all.sh --case w4a16_decode_fpA_intB
```
...
| dense W4A16 reference | `w4a16_prefill_cutlass55` |
| dense W4A16 reference | `w4a16_decode_fpA_intB` |
