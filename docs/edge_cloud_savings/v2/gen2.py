...
add('tag', '花十分钟读完，你会🉐到三件事：第一章，数据不出门是怎么做到的，你在哪里能看见它，以及为什么这事比省钱还靠前；第二章，云端账单到底能省多少；第三章，这套东西长什么样。')

TOC = [
    ('一、隐私：数据不出门，才敢把活儿交给 AI', ['1.1 为什么隐私排第一', '1.2 隐私是怎么保住的：一道门、一个门卫、一间小黑屋', '1.3 隐私在哪里看得见']),
    ('二、省钱：云端账单到底能省多少', ['2.1 先说清省的是哪本账', '2.2 钱是怎么省下来的：三次截流', '2.3 各场景能省多少', '2.4 换算成人民币']),
    ('三、架构：本地当店长，云端做会诊', ['3.1 业界方案长得都差不多：一个管「家」，一个管「路」', '3.2 一图看完：五步闭环', '3.3 谁干什么、不许干什么']),
    ('结论：让云端更贵重，而不是更常驻', []),
    ('附一：分场景完整数据', []),
    ('附二：数据来源', []),
]
...
add('img', 'priv_leak.png', 2.6)
add('p', '很多人一听「端云协同」，第一反应是省钱。其实同样重要的还有隐私：**数据不出门，才敢把活儿交给 AI。** 省钱和隐私不用二选一，这一章先讲怎么把门守住。')
...
add('p', '前两章说的「门」和「截流」，落到一台设备上是什么样？这一章讲清楚。业界已经有一些方案，架构都很接近：一个本地编排器当店长，一个路由器决定什么时候叫云端。')

add('h3', '3.1 业界方案长得都差不多：一个管「家」，一个管「路」')
add('p', '**管「家」的：以 Perplexity Portable Computer 为例。** 2026 年 2 月 Perplexity 发布了 Computer：一个会拆任务、派子代理、在沙箱里跑工具、还接 Slack / GitHub / 邮件的多模型 Agent。8 月它又和 NVIDIA 一起推出了完全本地版 Portable Computer：编排模型、子代理模型、Agent 运行时、工具沙箱、连接器，全部跑在你自己的硬件上（DGX Spark，或任何 24GB 显存以上的 RTX 卡），本地模型是一个专门为 Agent 后训练过的 27B。本地完成的步骤**不消耗一分钱云端额度**；真需要前沿推理时，先弹窗要你批准，出站内容先过隐私标记，云端只能回文字建议。沙箱是操作系统级强制隔离，沙箱不可用就直接禁用工具执行。第一章那三个词，在这类方案里都能找到对应。')
add('p', '**管「路」的：以 NVIDIA NeMo Switchyard 为例。** 它是 NVIDIA 开源的模型路由层，专门回答一个问题：Agent 的这一次调用，到底要不要花前沿模型的钱？它自带几种现成的路由器，大白话版：')
add('bullets', [
...
add('p', '第二章那些「还要上云 X%」，大多就是这些路由器在 Terminal-Bench、τ²-bench 和 LangChain 任务集上跑出来的。一句话：**「家」决定本地怎么布置，「路」决定什么时候该出门、出门带多少东西。** 我们的架构也是这两块拼起来的。')
...
add('p', '对技术决策者来说，稳妥的起点不是先喊「省 80%」，而是先立统一的评测框架和三本 Token 账，把门守住之后再一步步放大本地的比例。最终要证明的不是「本地模型很强」，而是：**在质量、延迟和隐私边界可接受的前提下，到底有多少活儿不必再让云端专家全程坐班。**')
add('hr')
