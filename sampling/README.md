...
|---|---|---|
| `lm_head` | cuBLAS GEMM via `general/bench_cublas_gemm` | `(1,3072) x (3072,248320) -> (1,248320)` |
...
The `bench_all.sh` lm-head case uses `general/bench_cublas_gemm`. `bench_sampling`
still includes a direct `--op=lm_head` cuBLAS comparison for local sampling-only
runs. Sampling stages call FlashInfer CUDA header kernels vendored under
