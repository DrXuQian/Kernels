...
// Compatibility note for this standalone extraction: some CUDA 12.x libcu++
// releases do not provide cuda::maximum<> / cuda::minimum<> yet.  FlashInfer
// only needs transparent binary functors for CUB reductions here, so keep the
// behavior local to this vendored header.
struct MaxReduceOp {
  template <typename T>
  __host__ __device__ __forceinline__ constexpr T operator()(T const& a, T const& b) const {
    return a < b ? b : a;
  }
};
