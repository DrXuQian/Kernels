 ...
 */
#ifndef MARLIN_MOE_KERNEL_PPU_CUH
#define MARLIN_MOE_KERNEL_PPU_CUH
...
namespace marlin_moe_kernel_ppu {
...
Marlin(const int4* __restrict__ A, const int4* __restrict__ B, int4* __restrict__ C,
       ...
       int* locks,
       // MoE: the expert of each m-tile, and the per-expert strides of B / s / mins in int4 units.
       // nullptr => every offset below is zero, so this stays a strict superset of the dense kernel.
       const int* __restrict__ mtile_expert, long long b_expert_stride, long long s_expert_stride) {
  int parallel = 1;
  ...
  // MoE lives HERE and nowhere else. Tokens are sorted by expert and each expert's segment is padded to
  // whole m-tiles, so an m-tile belongs to exactly one expert and the expert dimension IS Marlin's
  // `parallel` dimension. Nothing about the scheduler, the pipeline or the gs=32 group machinery changes;
  // only the B / s / mins bases move. Marlin already flattens (k_tile, n_tile, parallel) over a grid of
  // #CU and stripes it, so ONE launch covering every expert's tiles is already the persistent grouped
  // kernel -- there was no second scheduler to write, which is why this is a 3-edit derivation and not a
  // rewrite of the thing that took a whole session to get correct.
  int cur_e = mtile_expert ? mtile_expert[slice_col_par / n_tiles] : 0;
  B += cur_e * b_expert_stride;
  s += cur_e * s_expert_stride;
  if (AFFINE) mn += cur_e * s_expert_stride;
    ...
    if (slice_iters == 0) {
      ...
      if (slice_iters) {
        ...
        if (slice_col == 0) {
          ...
          // slice_col wrapping to 0 means we moved to the next m-tile, which may be a different expert.
          // B_ptr are absolute so they shift by the delta; s / mn are the bases s_gl_rd is relative to,
          // so they shift too and the s_gl_rd line below is unchanged.
          if (mtile_expert) {
            const int ne = mtile_expert[slice_col_par / n_tiles];
            if (ne != cur_e) {
              const long long db = (long long)(ne - cur_e) * b_expert_stride;
              #pragma unroll
              for (int i = 0; i < b_sh_wr_iters; i++) B_ptr[i] += db;
              s  += (long long)(ne - cur_e) * s_expert_stride;
              if (AFFINE) mn += (long long)(ne - cur_e) * s_expert_stride;
              cur_e = ne;
            }
          }
        }
...
#define CALL_IF_A(MB, NB, KB, GB, AFF) \
    ...
    Marlin<THREADS, MB, NB, KB, STAGES, GB, AFF><<<blocks, THREADS, shmem, stream>>>(A_ptr, B_ptr, C_ptr, s_ptr, m_ptr, prob_m, prob_n, prob_k, locks, mtile_expert, b_expert_stride, s_expert_stride); }
...
static int marlin_cuda(const void* A, const void* B, void* C, void* s, void* mins, int prob_m, int prob_n, int prob_k,
        ...
        const int* mtile_expert = nullptr, long long b_expert_stride = 0, long long s_expert_stride = 0) {
  ...
  for (int i = 0; i < tot_m_blocks; i += MB) {
    ...
    // mtile_expert is indexed by the par index WITHIN a launch (slice_col_par / n_tiles), so it advances
    // with A_ptr, not from the start of the plan. Missing this would silently give every launch after the
    // first the wrong experts -- the same class of bug as the segment padding, and just as quiet.
    if (mtile_expert) mtile_expert += par;
  }
  return ret;
}

// The m-tile the dispatcher will use, so moe_plan() pads to the SAME granularity. If the plan pads to a
// different size than the kernel tiles at, an m-tile straddles two experts and applies one expert's B to
// both -- wrong rows, no error. Tied to MARLIN_MAX_MB rather than restated as a literal.
inline int moe_m_tile(int prob_m) { return 16 * ((prob_m <= 16) ? 1 : MARLIN_MAX_MB); }
