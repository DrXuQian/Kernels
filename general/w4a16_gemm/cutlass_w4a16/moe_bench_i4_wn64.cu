// Single-plane i4 (int4), WarpN=64. Single plane means the delivery bound is just WN*TK*4 >= 4096, so
...
#define ROW(TMv, WMv)                                              \
  MOE1(bd, best, "i4", int4_t, 4, TMv,  64, MOE_TK, WMv, 64); \
  MOE1(bd, best, "i4", int4_t, 4, TMv, 128, MOE_TK, WMv, 64);
void sweep_i4_wn64(const Band& bd, Best& best) {
  std::printf("  --- i4 = single-plane int4, WN=64 --- [%s]\n", PPU_CHUNK_STR);
  MOE_GRID(ROW)
}
