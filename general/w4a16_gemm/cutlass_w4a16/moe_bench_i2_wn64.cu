// Single-plane i2 (int2), WarpN=64. Single plane means the delivery bound is just WN*TK*2 >= 4096, so
...
#define ROW(TMv, WMv)                                              \
  MOE1(bd, best, "i2", uint2_t, 2, TMv,  64, MOE_TK, WMv, 64); \
  MOE1(bd, best, "i2", uint2_t, 2, TMv, 128, MOE_TK, WMv, 64);
void sweep_i2_wn64(const Band& bd, Best& best) {
  std::printf("  --- i2 = single-plane int2, WN=64 --- [%s]\n", PPU_CHUNK_STR);
  MOE_GRID(ROW)
}
