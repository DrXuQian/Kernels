...
template <int MAX_COMMON_VECTOR, int FRAG_ELEMS, int GMEM_ELEMS> struct REPORT;
...
static constexpr int GMEM_ELEMS = int(cute::size(decltype(tCgA()){}));
REPORT<MCV, FRAG_ELEMS, GMEM_ELEMS> report;
