...
int main(int argc, char** argv) {
  ...
  // Uninitialized device buffers -- the probe path ignores the data (only needs valid pointers).
  ...
  { std::vector<half_t> z((size_t)L*Mmax*N, half_t(0.f)); D.copy_from_host(z.data()); }   // zero D so "unwritten" reads as 0
  ...
  for (int e=0;e<L;e++){
    ...
    for (int i=0;i<me[e];i++)
      for (int j=0;j<N;j++){
        int v = (int)std::lround((double)float(hD[(size_t)e*Mmax*N + (size_t)i*N + j]));
