# MoE Auxiliary Kernels

MoE FFN pipeline 中 GEMM 以外的辅助 kernel，从 vLLM 提取。

## Pipeline

```
topk_gating → moe_align → [GEMM gate_up] → silu_and_mul → [GEMM down] → moe_sum
```

## 编译

```bash
make
```

## 运行

```bash
./bench_topk_gating   [num_tokens] [num_experts] [topk]              # default: 1 64 8
./bench_moe_align     [num_tokens] [num_experts] [topk] [block_size] # default: 1 64 8 16
./bench_silu_and_mul  [num_tokens] [top_k] [hidden_size]             # default: 1 8 5632
./bench_moe_sum       [num_tokens] [topk] [hidden_size]              # default: 1 8 5632
```

## ncu Profile

```bash
ncu --set full --kernel-name "topkGating"    -o topk  ./bench_topk_gating 1 64 8
ncu --set full --kernel-name "moe_align"     -o align ./bench_moe_align 1 64 8 16
ncu --set full --kernel-name "silu_and_mul"  -o silu  ./bench_silu_and_mul 1 8 5632
ncu --set full --kernel-name "moe_sum"       -o sum   ./bench_moe_sum 1 8 5632
```
