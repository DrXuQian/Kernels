...
This file lists the Qwen3.5-122B-A10B operators covered by the standalone
benchmark folders and `bench_all.sh`.
...
Shared kernels are documented under the module that uses them.
...
| 2 | Top-K mask logits | `(1,248320)->(1,248320)` | `sampling/bench_sampling`, vendored FlashInfer kernel headers | `sampling_topk_mask_logits` |
| 3 | Softmax | `(1,248320)->(1,248320)` | `sampling/bench_sampling`, vendored FlashInfer kernel headers | `sampling_softmax` |
| 4 | Top-P sample | `(1,248320)->(1,)` | `sampling/bench_sampling`, vendored FlashInfer kernel headers | `sampling_top_p` |

## Shared Support

| Operator | Shape | Implementation | Used by |
|---|---:|---|---|
| FP16/BF16 GEMM | `M,N,K` | `general/bench_cublas_gemm` | Linear-Attn, MoE-FFN, Sampling |
| RMSNorm | `(tokens, hidden)` or `(tokens*heads, head_dim)` | `general/bench_rmsnorm.cu`, built into category folders | Flash-Attn, Linear-Attn, MoE-FFN |
| W4A16 prefill GEMM | `M,N,K,group=128` | `general/w4a16_gemm/machete_standalone`, CUTLASS55 backend | Flash-Attn, Linear-Attn, MoE-FFN |
| W4A16 decode GEMM | `M,N,K,group=128` | `general/w4a16_gemm/fpA_intB_standalone` | Flash-Attn, Linear-Attn, MoE-FFN |

## Coverage Answer

For the operator list above, `bench_all.sh --list` contains a case for every row.
