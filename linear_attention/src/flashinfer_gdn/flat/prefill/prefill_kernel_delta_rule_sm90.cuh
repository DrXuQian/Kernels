...
void launch_delta_rule_prefill_kernel_gbai(
  ...
  if constexpr (HopperSupported) {
    ...
    // C++17 compatible: nested add_option (no lambda in unevaluated context)
    using _IsGVA = std::conditional_t<IsGVA, cute::true_type, cute::false_type>;
    using _NeedsBeta = std::conditional_t<NeedsBeta, cute::true_type, cute::false_type>;
    using _NeedsAlpha = std::conditional_t<NeedsAlpha, cute::true_type, cute::false_type>;
    using _InitState = std::conditional_t<InitStateFromInput, cute::true_type, cute::false_type>;
    using _EnableCkpt = std::conditional_t<EnableCheckpointing, cute::true_type, cute::false_type>;
    using Options = decltype(
        add_option(Option<Tag::kEnableCheckpointing, _EnableCkpt>{},
        add_option(Option<Tag::kInitStateFromInput, _InitState>{},
        add_option(Option<Tag::kNeedsAlpha, _NeedsAlpha>{},
        add_option(Option<Tag::kNeedsBeta, _NeedsBeta>{},
        add_option(Option<Tag::kIsGVA, _IsGVA>{},
